...
export async function onRequestPost(context: any) { const request = context.request;
  try {
    ...
    if (!projectId) {
      return Response.json({ error: 'slug or projectId required' }, { status: 400 });
    }
    ...
    if (!version) {
      return Response.json({ error: `No compatible version found for ${GAME_VERSION} + ${LOADERS.join('/')}` }, { status: 404 });
    }
