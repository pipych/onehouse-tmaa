...
export default function MediaBlog({ currentUser, onProfileClick, isCreatingPost, setIsCreatingPost }: MediaBlogProps) {
  ...
  // Состояния интерактивности комментариев и лайков
  ...
  // Хранилища для раскрытия UI элементов
  ...
  // Загрузка комментариев и лайков к ним
  ...
  const publishPost = async () => {
    ...
    if (!newPostTitle.trim() || !postContent.trim() || !currentUser) return alert('Заголовок и контент пусты!');

    let finalPublishedAt = newPostPublishedAtInput ? new Date(newPostPublishedAtInput).toISOString() : new Date().toISOString();
    const postData = { author_id: currentUser.id, title: newPostTitle, content: postContent, cover_url: newPostCoverUrl || null, youtube_url: newPostYoutubeUrl || null, published_at: finalPublishedAt };
  ...
  // Вспомогательный рендер одного блока комментария (основного или дочернего)
    ...
    return (
      <div key={comment.id} className={`flex gap-3 items-start group/comment ${isReply ? 'mt-3 pl-4 border-l-2 border-white/5' : 'mt-5'}`}>
        ...
        <div className="flex-1 bg-white/[0.02] border border-white/5 p-3 rounded-2xl relative">
          ...
          {/* Логика скрытия длинного текста одной строкой */}
          ...
          {/* Кнопка стрелочки разворачивания */}
          {isLongText && (
            <button 
    ...
    return (
      <div className="w-full max-w-3xl mx-auto animate-fade-in pb-32 px-4 md:px-0 flex flex-col">
        ...
        <div className="w-full select-none flex" style={{ paddingTop: '20px', marginBottom: '44px' }}>
          <button onClick={handleClosePost} className="w-12 h-12 flex items-center justify-center bg-white/5 border border-white/10 rounded-full text-gray-300 hover:text-white transition-all active:scale-90 shadow-lg">
            <ArrowLeft size={20} />
          </button>
        </div>
        ...
        <div className="bg-[#14171c]/90 backdrop-blur-xl border border-white/5 rounded-[32px] overflow-hidden shadow-2xl flex flex-col pt-2 relative">
          <div className="p-5 md:p-6 pb-2 flex items-center justify-between select-none">
            <div className="flex items-center gap-4">
              <img src={selectedPost.author?.avatar_url || 'https://via.placeholder.com/150'} className="w-12 h-12 rounded-full object-cover" />
              <div>
                <div className="text-base font-bold text-white">{selectedPost.author?.rp_name || 'Неизвестный'}</div>
            ...
            {canManagePost(selectedPost) && (
              <button onClick={() => handleDeletePost(selectedPost.id)} className="p-2 text-red-400 hover:text-red-300 text-xs font-bold bg-red-500/10 rounded-xl">Удалить</button>
            )}
          </div>

          {selectedPost.cover_url && <img src={selectedPost.cover_url} className="w-full aspect-video object-cover px-5 md:px-6 rounded-2xl" loading="lazy" />}

          <div className="p-5 md:p-6 pt-4 flex flex-col gap-5">
            ...
            <div className="flex items-center justify-start gap-3 mt-4">
              ...
              <button onClick={(e) => handleSharePost(e, selectedPost.id)} className="flex items-center justify-center gap-2 px-4 py-2 bg-white/5 border border-white/5 rounded-full text-gray-400 hover:text-[#c0ff00] text-xs font-bold">
              ...
              topLevelComments.map(mainComment => {
                ...
                return (
                  <div key={mainComment.id} className="pb-4">
                    ...
                    {/* Кнопка показа/скрытия вложенных ответов */}
                    {hasReplies && (
                    ...
                    {/* Вложенный Тред ответов */}
                    {hasReplies && isThreadOpen && (
                      <div className="pl-8 animate-fade-in">
                        {replies.map(reply => renderCommentBlock(r, true))}
                      </div>
                    )}
                  </div>
                );
  ...
  // --------------------------------------------------------
  // КОД ЭКРАНА РЕДАКТИРОВАНИЯ И ГЛАВНОЙ ЛЕНТЫ
  // --------------------------------------------------------
      ...
      <div className="w-full max-w-3xl mx-auto animate-fade-in pb-40 px-4 md:px-0 flex flex-col pt-6">
        ...
        <div className="w-full" style={{ marginBottom: '54px' }}>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div onClick={handleDatePillClick} className={`relative flex items-center gap-2 border px-4 py-2 rounded-full cursor-pointer transition-all text-xs font-bold select-none ${newPostPublishedAtInput ? 'border-[#c0ff00]/40 bg-[#c0ff00]/10 text-[#c0ff00]' : 'bg-white/5 border-white/10 text-gray-400'}`}>
            ...
            <label className={`relative flex items-center gap-2 border px-4 py-2 rounded-full cursor-pointer transition-all text-xs font-bold select-none ${newPostCoverUrl ? 'border-[#c0ff00]/40 bg-[#c0ff00]/10 text-[#c0ff00]' : 'bg-white/5 border-white/10 text-gray-400'}`}>
              <input type="file" accept="image/*" className="absolute inset-0 opacity-0 cursor-pointer" onChange={(e) => handleFileUpload(e, setNewPostCoverUrl, setIsUploadingPostCover)} />
              <ImageIcon size={14} /> <span>Фото</span>
            </label>
          </div>
        </div>

        <input type="text" placeholder="Яркий заголовок..." value={newPostTitle} onChange={e => setNewPostTitle(e.target.value)} className="w-full bg-transparent text-3xl md:text-5xl font-black text-white border-none outline-none py-1 mb-8" />
        <div ref={editorRef} contentEditable className="w-full min-h-[40vh] bg-transparent text-lg text-gray-200 outline-none prose prose-invert focus:outline-none" data-placeholder="Текст вашей статьи..." />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in w-full max-w-3xl mx-auto px-2">
      ...
      <div className="flex flex-col gap-8 pb-8">
        {posts.map(post => (
          <div key={post.id} onClick={() => handleOpenPost(post)} className="bg-[#14171c]/90 backdrop-blur-xl border border-white/5 rounded-[32px] overflow-hidden shadow-2xl group cursor-pointer flex flex-col pt-2 relative">
            <div className="p-5 md:p-6 pb-2 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="text-base font-bold text-white">{post.author?.rp_name}</div>
            ...
            {post.cover_url && <img src={post.cover_url} className="w-full aspect-video object-cover px-5 md:px-6 rounded-2xl" loading="lazy" />}

            <div className="p-5 md:p-6 pt-4 flex flex-col gap-4">
              <div>
              ...
              <div className="flex items-center justify-start gap-3" onClick={e => e.stopPropagation()}>
                ...
                <button onClick={() => handleOpenPost(post)} className="flex items-center justify-center gap-2 px-4 py-2 bg-white/5 border border-white/5 rounded-full text-gray-400 hover:text-[#c0ff00] text-xs font-bold"><MessageCircle size={15} /></button>
              </div>
            </div>
          </div>
        ))}
      ...
      {/* ПАГИНАЦИЯ */}
      {totalPages > 1 && (
        <div className="flex justify-center items-center gap-2 select-none py-6">
          {Array.from({ length: totalPages }, (_, i) => i + 1).map(pageNum => (
            <button key={pageNum} onClick={() => handlePageSelect(pageNum)} className={`w-10 h-10 flex items-center justify-center rounded-full text-sm font-black ${currentPage === pageNum ? 'bg-[#c0ff00] text-black shadow-md' : 'bg-white/5 text-gray-500 hover:text-white'}`}>{pageNum}</button>
          ))}
        </div>
      )}
    </div>
